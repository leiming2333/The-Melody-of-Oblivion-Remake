...
- 面向渲染层的账户对象会移除 Microsoft 访问令牌和刷新令牌。
- Electron `safeStorage` 可用时，令牌会加密后落盘；如果操作系统无法提供安全存储，当前预览版会退回明文保存，因此不要在共享或不可信设备上使用 Microsoft 登录。
...
项目目前尚未添加许可证。在许可证发布前，仓库原创代码与资源适用默认版权限制。
