...
- **账户系统**：支持离线账户与 LittleSkin Yggdrasil 外置登录；可同步皮肤头像、启动前刷新在线凭据，并阻止令牌进入渲染进程。当前源码已移除 Microsoft 正版登录。
...
自动化测试目前覆盖账户、身份验证、下载、Java 选择、游戏启动、加载器、整合包、设置、本地版本管理及启动器更新等 87 个测试用例。
...
- 出于账户安全考虑，当前源码不提供 Microsoft 正版登录；如需正版登录请使用官方启动器。已发布的 v1.3.1 安装包不受此次源码改动影响。
...
- LittleSkin 的访问令牌和客户端令牌不会出现在提供给渲染进程的账户对象中。
- 在线账户令牌使用 Electron `safeStorage` 加密保存。如果安全存储不可用，启动器会拒绝保存或读取在线凭据，不会降级为明文存储。
- 当前源码不接入 Microsoft 正版登录，也不处理 Microsoft 账户凭据。
